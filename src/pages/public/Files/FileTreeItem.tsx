...
export default function FileTreeItem({ id, name, type, level, empty }: IProps): ReactElement {
  ...
  const ItemIcon = useMemo<React.FC>(() => {
    ...
    if (
      nameSplit.length === 1 ||
      nameSplit.length === 2 && !nameSplit[0]
    ) {
  ...
  return (
    <Box onClick={() => setActive(id)} sx={theme => ({
      display: 'flex',
      alignItems: 'center',
      flexFlow: 'row nowrap',
      py: 2,
      ...
      <Box component={ItemIcon} sx={theme => ({
        mr: 1,
        fontSize: '24px',
      ...
      {
        type === "folder" && !empty &&
        <NavigateNextIcon onClick={toggleOpen} sx={theme => ({
