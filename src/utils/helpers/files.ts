...
export const getFilteredFiles = (data: FileDataList, search: string): IFile[] => {
  ...
  data.forEach(item => {
    if (item.type === 'folder') {
      ...
      if (item.name.toLowerCase().split('.')[0].includes(search.toLowerCase())) {
        result.push(item)
      }
    }
  })
...
export const getFilteredFolderFiles = (folder: IFolder, search: string): IFile[] => {
  if (!search) {
    return []
  }

  const subFiles = folder.folders ? 
    folder.folders.map(subFolder => getFilteredFolderFiles(subFolder, search)) :
    []

  const files = folder.files ? 
    folder.files.filter(file => file.name.toLowerCase().split('.')[0].includes(search.toLowerCase())) :
    []
...
export const getFilteredData = (data: FileDataList, search: string): FileDataList | null => {
  ...
  return _.compact(
    data.map(item => {
      if (item.type === 'file') {
        return item.name.toLowerCase().split('.')[0].includes(search.toLowerCase()) ? item : null
      } else {
...
export const getFilteredFolder = (folder: IFolder, search: string): IFolder | null => {
  ...
  const noFilteredFolders = !filteredFolders || !_.compact(filteredFolders).length

  const noFilteredFiles = !filteredFiles?.length

  if (noFilteredFolders && noFilteredFiles) {
    return folder.name.toLowerCase().split('.')[0].includes(search.toLowerCase()) ? 
